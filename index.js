...
// ANSWER: I don't think you could get a better time complexity without knowing the answer before running the function. The function will have to keep running until the answer is found.
...
// ANSWER: Yes, it is possilbe to imporove this function's space complexity. We acheive this by removing
// the 'numbers2' array and using the existing 'numbers' array to get the sqare value on the number:

function getNumbersSquared2(number) {
  let num;
...
//    Both of these functions count how many letters are in the string passed in. However, the way each function stores and counts the number of letters are different.

//    The first function, countLetters, has a time complexity of O(1), fixed amount of memory, this is because the function stores a value for every letter of the alphabet,
//    even letters that arent found in the input string.

//    The second function, countLettersRoundTwo however, does not store a value for every letter, instead only stores a value for each letter that appears inside the
//    input string. I think this funciton has a space complexity of O(log n) as there are only 26 letters in the alphabet, capping the length of the 'letters' return object.
